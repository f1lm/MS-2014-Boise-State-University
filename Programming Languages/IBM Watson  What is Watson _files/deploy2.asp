...
//PLUGINS_LIST=simpleDeploy,genericEventsBridge,resizableChatWindow

if (typeof(lpMTagConfig.plugins)=='undefined') { lpMTagConfig.plugins = {};}
(function(){
try{var unit = lpMTagConfig.getLPVarValue( "unit" )}catch(e){var unit = lpUnit};
try{var lang = lpMTagConfig.getLPVarValue( "language" )}catch(e){var lang = lpLanguage};
try{var defaultStyle = lpMTagConfig.getLPVarValue( "defaultStyle" );}catch(e){var defaultStyle = lpMTagConfig.defaultStyle}
if(defaultStyle=='-v17'){
	lpMTagConfig.defaultInvite="chat-"+unit+"-"+lang+defaultStyle;
}
else{lpMTagConfig.defaultInvite='chat-'+unit+'-'+lang;}
lpMTagConfig.plugins['simpleDeploy'] = {
		name : 'simpleDeploy',
		config : {
			server : "sales.liveperson.net",
			buttons : [{"name":"chat-"+unit+"-"+lang+defaultStyle,"pid":"lpbutton","afterStartPage":true,'ovr':'lpMTagConfig.db1'},
			            {"name":"chat-"+unit+"2-"+lang+defaultStyle,"pid":"lpbutton2","afterStartPage":true,'ovr':'lpMTagConfig.db2'},
						{"name":"chat-"+unit+"3-"+lang+defaultStyle,"pid":"lpbutton3","afterStartPage":true,'ovr':'lpMTagConfig.db3'},
                                       {"name":"chat-"+unit+"-"+lang+"-rev","pid":"lpbuttonrev","afterStartPage":true,'ovr':'lpMTagConfig.db1'}
                                  ]
		}
};
lpMTagConfig.plugins['genericEventsBridge'] = {
	...
	config: {  
		...
		invStart: function(vars){
			if(defaultStyle=="-v17"){
                ibmweb.overlay.show("lpinvite");
                setTimeout(function(){dijit.byId('dialog_lpinvite')._onKey = function(){}},1000);
            }
		},
		invAccepted: function(vars){
			if(defaultStyle=="-v17"){
                ibmweb.overlay.hide("lpinvite");
            }
		},
...
lpMTagConfig.plugins['resizableChatWindow'] = {
        name: 'resizableChatWindow',
        config : { 
		    dbOvrObj:['lpMTagConfig.db1'],
		    height: 420,
		    width: 575,
...
if(typeof lpMTagConfig.pluginCode=="undefined")lpMTagConfig.pluginCode={};
lpMTagConfig.pluginCode.resizableChatWindow={ver:1,name:"resizableChatWindow",center:!1,left:null,top:null,height:450,width:395,resizable:1,centerInviteOnScreen:function(){if(window.screen&&window.screen.height&&window.screen.width)this.left=window.screen.width/2-this.width/2,this.top=window.screen.height/2-this.height/2},appendLocParams:function(a){this.center&&this.centerInviteOnScreen();this.left!=null&&(a+=",left="+this.left);this.top!=null&&(a+=",top="+this.top);return a},getWindowParams:function(){var a=
"";lpMTagConfig.isMobile||(a+="status=0, toolbar=0, location=0, menubar=0, directories=0, resizable="+this.resizable+", scrollbars=0",a+=",height="+this.height+", width="+this.width,a=this.appendLocParams(a));return a},start:function(){this.log(this.name+" start","DEBUG");var a=this.getWindowParams();lpMTagConfig.InviteOpenChat=function(b,c){a.length>0?window.open(b,c,a):window.open(b,c)};lpMTagConfig.InviteOpenMultiChannel=function(b,c,d,e){a.length>0?window.open(b,c,a,e):window.open(b,c)};lpMTagConfig.inviteVOSAcceptCall=
function(b,c){a.length>0?window.open(b,c,a):window.open(b,c)}},dbClicked:function(a){this.log(this.name+" dbClicked","DEBUG");var b=eval(a.objName);try{var c=this.getWindowParams(),d;switch(a.status){case "online":d=b.getActionURL("Available");break;case "busy":d=b.getActionURL("Busy");break;case "offline":d=b.getActionURL("Offline")}c.length>0?window.open(d,"chat"+lpMTagConfig.lpNumber,c):window.open(d,"chat"+lpMTagConfig.lpNumber)}catch(e){this.log("Plugin "+this.name+" exception in dbClicked:"+
e,"ERROR")}return!1}};lpMTagConfig.pluginHook.dynButtons[lpMTagConfig.pluginHook.dynButtons.length]={name:"dbClicked",src:"resizableChatWindow",run:function(a){return lpMTagConfig.pluginCode.resizableChatWindow.dbClicked(a)}};
