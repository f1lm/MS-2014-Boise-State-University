...
(function(j) {
    queryly = {
        ...
        QuerylyBind: function() {
            queryly.searchReferrer = document.referrer;
            queryly.visitorID = queryly.util.getVisitorID();
            var pageurl =document.URL;
            if (queryly.inline.docurl != ''){
                pageurl = queryly.inline.docurl;
            } 
            var apiurl = 'http://api.queryly.com/search.aspx?queryly_key=' + this.QuerylyKey + '&initialized=0&visitorid=' + queryly.visitorID + '&isdraft=' + queryly.isDraft + '&isphone=' + queryly.isPhone + '&templateid=' + queryly.templateID + '&relatedbatchsize=' + queryly.relatedBatchSize + '&random=' + Math.random() + '&pageurl=' + encodeURIComponent(pageurl);
...
//            }
    ...
    queryly.related = {
        ...
        loadAPI : function(){
            ...
            if (!queryly.related.processing){
                ...
                var relatedurl = 'http://related.queryly.com/json.aspx?queryly_key=' + queryly.QuerylyKey + '&callback=queryly.QuerylyRelatedCallback' + '&pageurl=' + encodeURIComponent(queryly.related.docurl) +'&decay=' + queryly.related.decay;
    ...
    queryly.campaign ={
        hookCampaignEvent: function(link,allocationid) {
            link.mousedown(function() {
                try {
                    new Image().src = "http://ad.queryly.com/track.aspx?queryly_key=" + queryly.QuerylyKey + "&visitorid=" + queryly.util.getVisitorID() + "&ad=1&clicked=1&allocationid=" + allocationid + "&pageurl=" + encodeURIComponent(document.URL.replace(/&amp;/g, "&"));
                }
                catch (e) { }
            });
        }
    ...
    queryly.render = {
        ...
        reposition: function() {
            if (queryly.customContainer != null && j('#' + queryly.customContainer).length == 1) {
                return;
            }
            queryly.util.setPositionInfo();
            var scrolltop = j(window).scrollTop();
...
if (queryly.isAndroid){
    ...
    jQuery.getScript( "http://www.queryly.com/js/touchswipe.js", function( data, textStatus, jqxhr ) {
    });
}
