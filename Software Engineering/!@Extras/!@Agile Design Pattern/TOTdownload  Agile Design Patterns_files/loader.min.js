...
if(this.is_object(this.options.plugins))for(b in this.options.plugins)this.is_defined(this.plugin_helpers[b])&&(c=c||{},c[b]=this.extend(!0,{},this.plugin_helpers[b]._defaults,this.options.plugins[b],this.plugin_helpers[b]));this.options.plugins=c||{related:this.extend(!0,{},this.plugin_helpers.related._defaults,this.plugin_helpers.related)};this.plugins_sanitized=!0;for(b in this.options.plugins)this.snt(b);this.lcs();this.debug("Options initialized %o",this.options)},snt:function(a){var b,c,d,e=
["cssstyle","thumbsize","ad_place","widgetstyle"];c={style:"cssstyle",thumb_size:"thumbsize"};for(b in c)this.is_defined(this.options.plugins[a][b])&&(this.options.plugins[a][c[b]]=this.options.plugins[a][b],delete this.options.plugins[a][b]);b=0;for(c=e.length;b<c;b++)d="supported_"+e[b]+"s",0>this.in_array(this.options.plugins[a][e[b]],this.options.plugins[a][d])&&(this.options.plugins[a][e[b]]=this.options.plugins[a][d][0]);this.is_function(this.options.plugins[a].csnt)&&this.options.plugins[a].csnt.call(this,
this.options.plugins[a])},lcs:function(){var a=this.options.debug_mode?this.options.debug_cs_api_url:this.options.cs_api_url,a=a+(this.to_slug(this.options.domain)+".js");this.debug("Loading custom scripts from %o",a);this.lr(a,"nrelate-widget-cs");var b=this;setTimeout(function(){b.acs({})},5E3)},acs:function(a){if(this.plugins_sanitized&&!this.custom_scripts_loaded){this.custom_scripts_loaded=!0;var b;this.debug("Applying custom scripts");for(b in this.options.plugins)this.extend(this.options.plugins[b],
a[b]);this.nr_cs_load_time=(new Date).getTime();if(!this.options.browser.msie)try{this.pd()}catch(c){}this.bdr(this.pd,"pd()")}},pd:function(){var a,b,c,d,e;for(p in this.options.plugins){this.is_function(this.options.plugins[p].fph)?(a=this.options.plugins[p].fph,this.debug("Use custom fph for %s",p)):(a=this.dfph,this.debug("Use default fph for %s",p));this.is_function(this.options.plugins[p].fpl)?(b=this.options.plugins[p].fpl,this.debug("Use custom fpl for %s",p)):(b=this.dfpl,this.debug("Use default fpl for %s",
p));this.is_function(this.options.plugins[p].fkw)?(c=this.options.plugins[p].fkw,this.debug("Use custom fkw for %s",p)):(c=this.dfkw,this.debug("Use default fkw for %s",p));this.debug("Parsing document for %s",p);d=this.options.plugins[p].cts.length;this.options.plugins[p].phs=a.call(this,p);this.options.plugins[p].pls=b.call(this,p);e={};a=0;for(b=this.options.plugins[p].pls.length;a<b;a++)e[this.options.plugins[p].pls[a]]=1;this.is_home_page=Boolean(this.is_defined(window.nr_is_home)?window.nr_is_home:
1<this.get_keys(e).length);this.options.plugins[p].kws=c.call(this,p);a=d;for(b=this.options.plugins[p].phs.length;a<b&&a<(1>=b?1:15);++a)this.act(a,p);this.mac()}},dfph:function(a){var b,c,d,e,g,h,m,l,f=[],r="post-body article-content entry-content entry post-entry post-inner postmeta2 post_content text postcontent single post-header content post art-PostContent jump-link entrytitle wrap blogPost post-footer postfooter commentpage post-footer-line".split(" "),n=["post-footer","postfooter","commentpage",
"post-footer-line"],q=["format_teaser","sideContent","ngg-widget"];if(f=this.xgebi("nrelate_"+a+"_placeholder"))return this.debug("Backwards compatibility: using #nrelate_%s_placeholder: %o",a,f),[f];if(0<(f=this.xgebcn("nr_"+a+"_placeholder","div")).length)return this.debug("Using manually located .nr_%s_placeholder: %o",a,f),f;if(this.options.manual_ph_only)return[];b=0;for(e=r.length;b<e;b++)if(m=this.xgebcn(r[b]),0<(g=m.length)){this.debug("Found candidates with .%s selector",r[b]);for(c=0;c<
g;c++){d=0;for(h=q.length;d<h;d++)if(this.xhc(m[c],q[d])){this.debug("%o excluded because of class .%s",m[c],q[d]);m[c]=!1;break}m[c]&&0<=this.in_array(r[b],n)&&(this.is_defined(m[c].previousSibling)&&1===m[c].previousSibling.nodeType?(this.debug("Asigning previousSibling because of .%s",r[b]),m[c]=m[c].previousSibling):(this.debug("%o excluded because it doesn't have valid previousSibling",m[c]),m[c]=!1));m[c]&&"post-body"==r[b]&&0<(l=this.xgebcn("jump-link","*",m[c].parentNode)).length&&(m[c]=l[0]);
m[c]&&(f[f.length]=m[c])}if(f.length)return this.debug("Document parsed. Found these placeholders %o",f),f}else this.debug("No placeholders found with .%s selector",r[b]);if(f=this.xgebi("nrelate_"+a+"_backup_placeholder"))return this.debug("Backwards compatibility: using #nrelate_%s_backup_placeholder: %o",a,f),[f];this.debug("No placeholders found");return[]},dfpl:function(a){var b,c,d,e,g,h,f,l=[],n=["entry-title","post-title","entry-header"],r=["h3","h2","h1"];if(1==this.options.plugins[a].phs.length&&
"undefined"!=typeof window.nr_pageurl)return this.debug("Manually defined nr_pageurl"),[{href:window.nr_pageurl}];if(0<this.options.plugins[a].phs.length&&this.options.plugins[a].phs[0].getAttribute("data-permalink")){b=0;for(e=this.options.plugins[a].phs.length;b<e;b++)if(g=this.get_data_attr(this.options.plugins[a].phs[b],"permalink"))l[l.length]={href:g};else{l=[];break}if(l.length)return this.debug("Found data-permalink attribute on placeholders"),l}if(1<(l=this.xgeba("a","title","permanent link")).length)return this.debug("Found title='permanent link' permalinks %o",
l),l;b=0;for(e=n.length;b<e;b++)for(c=0,g=r.length;c<g;c++){if(0<(h=this.xgebcn(n[b],r[c])).length)for(d=0;d<h.length;d++)0<(f=this.xgebtn("a",h[d])).length&&(l[l.length]=f[0]);if(0<l.length)return this.debug("Found permalinks on %s.%s %o",r[c],n[b],l),l}if(0<(l=this.xgeba("a","rel","bookmark")).length)return this.debug("Found rel='bookmark' permalinks %o",l),l;if(0<(l=this.xgebcn("permalink","a")).length)return this.debug("Found class='permalink' permalinks %o",l),l;if(0<(c=this.options.plugins[a].phs).length){b=
0;for(e=c.length;b<e;b++)0<(f=this.xgebtn("a",c[b])).length&&(l[l.length]=f[0]);if(0<l.length)return this.debug("Found first links inside placeholders %o",l),l}if(0<(f=this.xgeba("meta","property","og:url")).length){b=0;for(e=this.options.plugins[a].phs.length;b<e;b++)l[l.length]={href:f[0].content};this.debug("Found og:url meta tag");return l}if(0<(f=this.xgeba("link","rel","canonical")).length){b=0;for(e=this.options.plugins[a].phs.length;b<e;b++)l[l.length]=f[0];this.debug("Found canonical link tag");
return l}b=0;for(e=this.options.plugins[a].phs.length;b<e;b++)l[l.length]={href:window.location.href};this.debug("use window.location.href");return l},dfkw:function(a){var b,c,d,e,g,h,f=document.title,l=[],n=["entry-title","post-title","entry-header"],r=["h3","h2","h1"];if("undefined"!=typeof window.nr_keywords&&"encodeURI(document.title)"!=window.nr_keywords)return this.debug("Manually defined nr_keywords"),[window.nr_keywords];if(0<this.options.plugins[a].phs.length&&this.options.plugins[a].phs[0].getAttribute("data-title")){b=
0;for(c=this.options.plugins[a].phs.length;b<c;b++)l[l.length]=this.get_data_attr(this.options.plugins[a].phs[b],"title");this.debug("Found data-title attribute on placeholders");return l}b=0;for(c=n.length;b<c;b++)for(d=0,e=r.length;d<e;d++){if(0<(g=this.xgebcn(n[b],r[d])).length)for(k=0;k<g.length;k++)if(h=g[k].innerText||g[k].textContent,this.is_home_page)l[l.length]=h;else if(0<=f.search(this.regex_escape(h))){b=0;for(c=this.options.plugins[a].pls.length;b<c;b++)l[b]=h;break}if(0<l.length)return this.debug("Found keywords on %s.%s %o",
r[d],n[b],l),l}if(0<(d=this.xgeba("meta","property","og:title")).length){b=0;for(c=this.options.plugins[a].phs.length;b<c;b++)l[l.length]=d[0].content;this.debug("Found og:title meta tag");return l}if(!this.is_home_page){b=0;for(c=this.options.plugins[a].pls.length;b<c;b++)l[b]=f;this.debug("Using document.title as keyword");return l}this.debug("No keywords found");return[]},act:function(a,b,c){var d;c="string"!=typeof c?"":c;d=document.createElement("div");d.id="nrelate_"+b+"_"+a;d.innerHTML=c;this.xac(d,
"nrelate");this.xac(d,"nrelate_"+b);this.xac(d,"nr_clear");d.settings={};d.settings.api_called=!1;d.settings.plugin=b;d.settings.plugin_shortname=this.options.plugins[b].shortname;d.settings.viewed=!1;this.options.plugins[b].phs[a].appendChild(d);this.options.plugins[b].cts[a]=d},mac:function(){var a,b,c,d,e,g,h={};for(a in this.options.plugins)if(this.is_function(this.options.plugins[a].should_load)&&!this.options.plugins[a].should_load.call(this,this.options.plugins[a]))this.debug("Ignore %s API calls because of should_load",
a);else for(h[a]=!0,this.debug("Making API calls for %s",a),b=0,c=this.options.plugins[a].cts.length;b<c;b++)if(!(d=this.options.plugins[a].cts[b]).settings.api_called){e=1==this.options.plugins[a].phs.length?document.title:"";e=this.aurlp(this.options.plugins[a].api_url,{plugin:d.settings.plugin_shortname,tag:"nrelate_"+a,domain:this.options.domain,keywords:this.options.plugins[a].kws[b]||e,url:this.surl(this.options.plugins[a].pls[b].href),nr_div_number:b,pr_id:this.get_print_id()});h[a]&&(e=this.aurlp(e,
{count_page:Number(h[a])}));"referrer"in document&&document.referrer&&(e=this.aurlp(e,{referrer:document.referrer}));this.is_home_page||(d.settings.src_url=this.options.plugins[a].pls[b].href);e=this.apip(this.options.plugins[a],e,d);if(g=this.gurlp(window.location.search,"predefined_set_override"))e=this.aurlp(e,{predefined_set_override:g});this.nr_parse_time=(new Date).getTime();if("performance"in window){g=this.nr_load_time-window.performance.timing.responseStart;var f=this.nr_cs_load_time-this.nr_load_time,
l=this.nr_parse_time-Math.max(window.performance.timing.domInteractive,this.nr_cs_load_time);this.debug("#%s Loading time: %f | CS Loading time: %f | Parse time %f",d.id,g/1E3,f/1E3,l/1E3);e=this.aurlp(e,{loading_time:g,cs_time:f,parse_time:l})}if(g=this.parse_json(this.get_data_attr(d.parentNode,"options")))e=this.aurlp(e,g);d.settings.api_call_time=(new Date).getTime();this.debug("Making API call for #%s: %o",d.id,e);this.lr(e,d.id+"-api_call")&&(h[a]=!1);d.settings.api_called=!0;d.settings.api_url=
e}},apip:function(a,b,c){var d,e,g={},h="cssstyle thumbsize ad_place widgetstyle header ad_header widget_id page_type_id page_type geo article_id".split(" "),f=["norelatedposts","ad_num"];this.is_array(this.options.extra_apip.general)&&(h=h.concat(this.options.extra_apip.general));d=0;for(e=h.length;d<e;d++)null!==a[h[d]]&&this.is_defined(a[h[d]])&&(g[h[d]]=a[h[d]]);this.is_array(this.options.extra_apip.numeric)&&(f=f.concat(this.options.extra_apip.numeric));d=0;for(e=f.length;d<e;d++)isNaN(a[f[d]])||
(g[f[d]]=a[f[d]]);g.length!=={}&&(b=this.aurlp(b,g));this.is_function(a.capip)&&(b=a.capip.call(this,b,a,c));return b},sw:function(a,b,c){var d=c||{};(c=this.xgebi(b))?(this.debug("Injecting content into #%s container",b),c.settings.count_views=d.count_views||this.options.plugins[c.settings.plugin].count_views,c.settings.src_did=d.did,d.plugin_shortname&&(c.settings.plugin_shortname=d.plugin_shortname),this.sctc(this.options.plugins[c.settings.plugin],c,d),c.innerHTML=a,this.sct(this.options.plugins[c.settings.plugin],
c,d),0==this.xgebcn("nr_link","a",c).length&&(c.style.display="none"),c.settings.widget_initialized_time=(new Date).getTime()):this.debug("#%s container not found",b)},sctc:function(a,b,c){b.settings.cssstyle=c.cssstyle||c.style||a.cssstyle||a.supported_cssstyles[1];b.settings.cssversion=c.cssversion||0;b.settings.thumbsize=c.thumbsize||c.thumb_size||a.thumbsize||a.supported_thumbsizes[1];b.settings.ad_place=c.ad_place||a.ad_place||a.supported_ad_places[1];b.settings.widgetstyle=c.widgetstyle||a.widgetstyle||
a.supported_widgetstyles[1];b.settings.whats_this_link=c.whats_this_link||a.whats_this_link||!1;b.settings.widget_id=a.widget_id=c.widget_id||"0";b.settings.page_type_id=c.page_type_id||a.page_type_id||"0";b.settings.page_type=c.page_type||a.page_type||"0";b.settings.geo=c.geo||a.geo||"0";b.settings.article_id=c.article_id||a.article_id||"0";if(b.settings.cssstyle)if("custom"==b.settings.cssstyle){c=this.to_slug(this.options.domain);var d=(this.options.debug_mode?this.options.debug_css_api_url:this.options.css_api_url)+
c+".css",d=this.aurlp(d,{v:b.settings.cssversion});this.debug("Loading custom style from %o",d);this.lr(d,"nrelate-"+c+"custom-style",{type:"css",callback:this.fah})}else"none"!=b.settings.cssstyle&&this.lr(this.options.default_stylesheet_url,"nrelate-styles",{type:"css",callback:this.fah});this.bind(window,"load",function(){nRelate.fah()});this.xac(b,"nrelate_"+b.settings.cssstyle);0===b.settings.widgetstyle?this.xac(b,"nr_text"):this.xac(b,"nr_"+b.settings.thumbsize);"Separate"==b.settings.ad_place?
a.cols_layout?this.xac(b,"nr_"+a.cols_layout):this.xac(b,"nr_2col"):this.xac(b,"nr_1row")},sct:function(a,b,c){var d=this;this.xac(b,"nr_initializing");(c.widget_id||b.settings.widget_id)&&this.xac(b,"widget_id_"+(c.widget_id||b.settings.widget_id));b.settings.cols_num=c.cols_num||0;this.fh(b);this.ctr(b);this.aa(b);this.pr(b);b.settings.whats_this_link&&this.wtl(b,a,c);c.uc&&this.ucs(b,a,c);this.is_function(a.csct)&&a.csct.call(this,a,b,c);this.xrc(b,"nr_initializing");this.xac(b,"nr_setup");this.bvc();
this.bdr(function(){d.cab(b)})},fh:function(a){if((a=this.xgebi(a))&&1!=a.settings.fixing_height){var b;if(0==(b=this.xgebcn("nr_panel","a",a)).length){if(0<(b=this.xgebcn("nr_link","a",a)).length)for(c=0;c<b.length;c++)this.xac(b[c].parentNode,"nr_li_"+(c+1)),this.xac(b[c],"nr_row_"+(c+1))}else{a.settings.fixing_height=!0;var c,d,e,g,h;num_rows=num_cols=img_height=link_height=link_y=d=0;row_links=[];tallest=0;_col_counter=_row_counter=1;h=this.xgebtn("img",a);c=0;for(e=h.length;c<e;c++)img_height=
...
!1;a.cancelBubble=!0;a.stopPropagation&&(a.stopPropagation(),a.preventDefault())},debug:function(){if(this.options.debug_mode&&window.console)if(this.options.browser.msie)try{console.log.apply(console,Array.prototype.slice.call(arguments))}catch(a){console.log(Array.prototype.slice.call(arguments))}else console.log.apply(console,Array.prototype.slice.call(arguments))},extend:function(){var a,b,c,d,e,f=arguments[0]||{},h=1,m=arguments.length,l=!1;"boolean"===typeof f&&(l=f,f=arguments[1]||{},h=2);
for("object"===typeof f||this.is_function(f)||(f={});h<m;h++)if(null!=(e=arguments[h]))for(d in e)a=f[d],c=e[d],f!==c&&(l&&c&&(this.is_plain_object(c)||(b=this.is_array(c)))?(b?(b=!1,a=a&&this.is_array(a)?a:[]):a=a&&this.is_plain_object(a)?a:{},f[d]=this.extend(l,a,c)):void 0!==c&&(f[d]=c));return f},to_slug:function(a){var b="";"string"==typeof a&&(b=a.replace(/[^a-zA-Z0-9]+/g,"-"));return b},get_data_attr:function(a,b){var c;if(c=this.xgebi(a))if(c=c.getAttribute("data-"+b))if("javascript:"==c.substr(0,
...
"%27"):"")+"]="+encodeURIComponent(b[c][e]).replace(/'/g,"%27");else a+=f+c+"="+encodeURIComponent(b[c]).replace(/'/g,"%27");f="&"}return a},rurlp:function(a,b){var c=new RegExp("\\?"+b+"=[^&]*&?","gi"),d;"*"==b?0<=(d=this.in_array("?",a))&&(a=a.substring(0,d)):(a=a.replace(c,"?"),c=new RegExp("\\&"+b+"=[^&]*&?","gi"),a=a.replace(c,"&"),a=a.replace(/(\?|&)$/,""));return a},gurlp:function(a,b){var c=a.replace("?","&").match(new RegExp("\\&"+b+"=([^&]*)&?"));return c&&0<c.length?c[1]:null},surl:function(a){var b,
...
"_"+b.settings.location);b.settings.default_stylesheet_url="http://css.nrcdn.com/common_js/0.52.0/nrelate-flyout-"+b.settings.animstyle+".min.css";this.lr(b.settings.default_stylesheet_url,"nrelate-fo-"+b.settings.animstyle,"css");b.settings.width=c.width||a.width||360;b.settings.widthtype=c.widthtype||a.widthtype||"px";b.settings.frombot=c.frombot||a.frombot||0;b.settings.frombottype=c.frombottype||a.frombottype||"px";b.style.width=b.settings.width+b.settings.widthtype;b.style.bottom=b.settings.frombot+
b.settings.frombottype;(fo_open=this.xgebi("nrelate_flyout_open"))||(fo_open=document.createElement("div"),fo_open.id="nrelate_flyout_open",this.xac(fo_open,"nrelate_animate_style_"+b.settings.animstyle),this.xac(fo_open,"nr_"+b.settings.animation+"_"+b.settings.location),document.body.insertBefore(fo_open,b.nextSibling),this.bind(fo_open,"click",function(){a.fo_animate_open.call(e,a,b);a.fo_animate_close.call(e,a,b,fo_open);t.del("nr_fo_closed")}));b.style.display="none";b.style[b.settings.location]=
"0px";fo_open.style.display="none";fo_open.style[b.settings.location]="0px";fo_open.style.bottom=b.settings.frombot+b.settings.frombottype;if(d=this.xgebi("nrelate_flyout_close"))d.style.background="#fff url(http://staticjs.nrcdn.com/common_js/close_window.gif) no-repeat 0 0",this.bind(d,"click",function(){a.fo_animate_close.call(e,a,b);a.fo_animate_open.call(e,a,b,fo_open);t.set("nr_fo_closed","true",7,"/")});b.settings.offset=c.offset||a.offset||1;b.settings.element=c.element||a.element||null;a.hscroll.call(e,
null,a,b);this.bind(window,"scroll",function(c){a.hscroll.call(e,c,a,b)})},hscroll:function(a,b,c){var d;if(!nRelate.flyout_show)return!1;a=b.should_open.call(this,b,c);if("true"==t.get("nr_fo_closed")){if(!(d=this.xgebi("nrelate_flyout_open")))return!1;a?b.fo_animate_open.call(this,b,c,d):b.fo_animate_close.call(this,b,c,d);return!1}a?b.fo_animate_open.call(this,b,c):b.fo_animate_close.call(this,b,c)},fo_animate_open:function(a,b,c){a=new s;var d=this;c=c||b;if(this.xhc(c,"nr_animating|nr_fo_opened"))return!1;
this.xac(c,"nr_animating");this.xrc(c,"nr_fo_opened|nr_fo_closed");c.style.display="block";"slideout"==b.settings.animation?(c.style[b.settings.location]="-"+(this.xWidth(c)+40)+"px",a.css(c,b.settings.location,0,400,5,0,function(){d.xrc(this.e,"nr_animating");d.xac(this.e,"nr_fo_opened")})):"fade"==b.settings.animation&&a.opacity(c,1,400,5,0,function(){d.xrc(this.e,"nr_animating");d.xac(this.e,"nr_fo_opened")})},fo_animate_close:function(a,b,c){a=new s;var d=this;c=c||b;if(this.xhc(c,"nr_animating|nr_fo_closed"))return!1;
...
4))?!1:!0},fo_faph:function(a){a=this.dfph(a.fullname)[0];this.debug("fo_faph result: %o",a);return a},fph:function(a){return[document.body]}})};s.prototype.axes=function(a){var b;if(!this.a||this.a.length!=a)for(this.a=[],b=0;b<a;++b)this.a[b]={i:0,t:0,d:0,v:0}};s.prototype.init=function(a,b,c,d,e,g,h){this.e=f.xgebi(a);this.t=b;this.or=c;this.ot=d;this.oe=e;this.at=g||0;this.v=s.vf[this.at];this.qc=1+(h||0);this.fq=1/this.t;0<this.at&&4>this.at&&(this.fq=this.fq*this.qc*Math.PI,1==this.at||2==this.at)&&
(this.fq/=2);for(a=0;a<this.a.length;++a)this.a[a].d=this.a[a].t-this.a[a].i};s.prototype.run=function(a){var b,c,d,e=this;a||(e.t1=(new Date).getTime());e.tmr||(e.tmr=setInterval(function(){e.et=(new Date).getTime()-e.t1;if(e.et<e.t){e.f=e.v(e.et*e.fq);for(b=0;b<e.a.length;++b)e.a[b].v=e.a[b].d*e.f+e.a[b].i;e.or(e)}else{clearInterval(e.tmr);e.tmr=null;c=e.qc%2;for(b=0;b<e.a.length;++b)e.a[b].v=c?e.a[b].t:e.a[b].i;e.ot(e);d=!1;"function"==typeof e.oe?d=e.oe(e):"string"==typeof e.oe&&(d=eval(e.oe));
d&&e.resume(1)}},e.res))};s.prototype.pause=function(){clearInterval(this.tmr);this.tmr=null};s.prototype.resume=function(a){"undefined"==typeof this.tmr||this.tmr||(this.t1=(new Date).getTime(),a||(this.t1-=this.et),this.run(!a))};s.prototype.css=function(a,b,c,d,e,g,h){function m(a){a.e.style[a.prop]=Math.round(a.a[0].v)+"px"}this.axes(1);this.a[0].i=f.xGetComputedStyle(a,b,!0);this.a[0].t=c;this.prop=f.xCamelize(b);this.init(a,d,m,m,h,e,g);this.run()};s.prototype.opacity=function(a,b,c,d,e,g){function h(a){f.xOpacity(a.e,
a.a[0].v)}this.axes(1);this.a[0].i=f.xOpacity(a);this.a[0].t=b;this.init(a,c,h,h,g,d,e);this.run()};s.vf=[function(a){return a},function(a){return Math.abs(Math.sin(a))},function(a){return 1-Math.abs(Math.cos(a))},function(a){return(1-Math.cos(a))/2},function(a){return 1-Math.exp(6*-a)},function(a){return-Math.cos(a*Math.PI)/2+.5}];var t={get:function(a){return(a=document.cookie.match(new RegExp("(^|;)\\s*"+a+"=([^;\\s]*)")))&&3<=a.length?unescape(a[2]):null},set:function(a,b,c,d,e,f){if(c){var h=
