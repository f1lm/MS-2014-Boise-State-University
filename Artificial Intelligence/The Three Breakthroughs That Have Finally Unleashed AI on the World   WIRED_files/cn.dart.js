...
CN.dart = window.CN.dart = (function($, CN, $D) {
    ...
    var
        ...
        call = function(name, pars) {
            ...
            if(isAdAlreadyCalled === false || gptAdsRendered === true){
                ...
                if(gptAdsRendered === true){
                    ...
                    if( $("#"+ interstitialPlacement1).length === 0){
       ...
       setOmnitureDFPVal = function(){
         ...
         * @param  {number}    exp          Length of time for the cookie, set as number of hours
         *                                  Can lead to some funky decimals if using fractions of an hour
         *                                  Does not support session setting
         * @param  {String}    commonVar    name of the property in the common object
         *
         * @private
         */
        buildTracking = function( name, exp, commonVar ) {
            var
                sqt = {},
                data = cookie.read( name ),
                expirationTime = exp * (1000 * 60 * 60),
            ...
            if (!data) {
                ...
                sqt.expire = timestamp;

                setExpire = timestamp / (1000 * 60 * 60);
                ...
                count = parseInt(sqt.count) + 1;

                common[commonVar] = '' + count;
                sqt.count = count;
        ...
        * @private
        ...
        getTrackingSeq = function() {
            ...
            buildTracking( "cn_adsqt", 23, "adSeqTargeting" );
            buildTracking( "cn_adcap", .1666666, "adSeqCap" );
        },
        ...
        refresh = function(placement, pars) {
            ...
            if (pars) {
                contentType = pars.contentType || "";
                setTargeting( "cttp", contentType );
            }
                ...
                if(placement && placements.length !== 0){
                    for(var ind1=0; ind1<placements.length; ind1++){
                        for(var ind=0; ind<moreAdSlots.length; ind++){
                            if(moreAdSlots[ind].placement.toString() === placements[ind1].toString()){
                                adSlots.push(moreAdSlots[ind].slot[0]);
                                $D.info(msg_pre+'refreshing ' + moreAdSlots[ind].placement +' moreAds');
        ...
        getAdLog : function(){
            ...
            gptAds = CN.dart.getGPTAds();
                ...
                for(var ind=0; ind<gptAds.length; ind++){
                    ...
                    renderedAds[gptAds[ind].adplacement] = {    'adEmpty' : common.gptEvent[ind].isEmpty ,
                                                                'creativeID' : common.gptEvent[ind].creativeId,
                                                                'linedItem' : common.gptEvent[ind].lineItemId,
                                                                'adSize' : common.gptEvent[ind].size,
                                                            };
