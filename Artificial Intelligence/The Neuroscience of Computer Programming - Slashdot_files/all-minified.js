...
function trackLinkDiceJob(link,category,action){_gaq.push(['_trackEvent','DiceJob',category,action]);_gaq.push(['b._trackEvent','DiceJob',category,action]);if(link&&link.href){setTimeout('document.location = "'+link.href+'"',100);}}
var modal=(function(){var
method={},$overlay,$modal,$header,$content,$footer,$close;method.center=function(){var top,left;top=Math.max($(window).height()-$modal.outerHeight(),0)/2;left=Math.max($(window).width()-$modal.outerWidth(),0)/2;$modal.css({top:top+$(window).scrollTop(),left:left+$(window).scrollLeft()});};method.open=function(settings){$modal.css({width:settings.width||'auto',height:settings.height||'auto'});method.center();$(window).bind('resize.modal',method.center);$modal.show();$overlay.show();};method.close=function(){$modal.hide();$overlay.hide();$(window).unbind('resize.modal');};function init(){$overlay=$('.overlay').hide();$modal=$('.modal-box').hide();$header=$('.modal-box header');$close=$('#close-modal');$close.click(function(e){e.preventDefault();method.close();});$overlay.click(function(e){e.preventDefault();method.close();});}
$(init);return method;}());function newsletterSignup(){modal.open({width:'660px',height:'520px',});$('.modal-box article').html('<iframe src="http://slashdotheadlines.p.slashdot.org/slashdot_daily_newsletter.html" height="450" width="630" scrolling="no" style="padding-top: 15px; overflow: hidden; border: none;"></iframe>');}
...
Slash.busy('firehose-update',false);firehose_add_update_timerid(setTimeout(firehose_get_updates,getFirehoseUpdateInterval()));saved_selection.restore().focus();$menu.show();}
...
function edit_editon(id,type,label){$('#firehose-'+id).hide();$('.editonly').removeClass('hide');$('.previewonly').addClass('hide');$('#editor').removeClass('step2').addClass('step1');Tags.fetch($('#editor'));$('#extra-warnings').addClass('hide');$('#edit-edit-save').hide();$('#edit_or').hide();$('.captcha').addClass("hide");if(type!==""&&type!==undefined&&label!==""&&label!==undefined){$(type+"[name='"+label+"']").focus();}}
...
function firehose_community_pick(id,val){ajax_update({op:'firehose_community_pick',id:id,val:val,reskey:reskey_static});}
