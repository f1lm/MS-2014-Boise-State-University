/* SiteCatalyst code version: H.24.3. Copyright 1996-2011 Adobe, Inc. All Rights Reserved More info available at http://www.omniture.com (td 2014-09-15) */
...
//s_time.Media.trackVars="events,eVar6,eVar7,prop51,eVar56,eVar57,eVar58,eVar59,eVar60,channel,server,contextData.bc_tags,contextData.bc_channel"; 
s_time.Media.trackVars="events,eVar6,eVar7,prop48,eVar52,eVar53,eVar54,eVar56,eVar57,eVar58,eVar59,eVar60,channel,server,contextData.bc_tags,contextData.bc_channel";
// 9/24/13 Time uses 51 not 48 for pathing
...
//console.log('>> ****************************************');
console.log('   onPlay mediaOffset value = "'+mediaOffset+'" evt.position="'+evt.position+'"');
//console.log('>> ****************************************');
...
/* Tags -----------------------------------------------------*/
  ...
  //console.log('> set defaults for franchise, etc.');
  var media_franchise    = 'franchise unavailable'; // default value if franchise is not available
  var media_subfranchise = 'subfranchise unavailable'; // default value if subfranchise is not available
  var get_published_date = 'published date unvailable'; 
   
  media_customFields=evt.media.customFields;
  
  if(typeof media_customFields.section=='string') { 
     media_subfranchise=media_customFields.section.toLowerCase();
	 console.log('>>>> section "'+media_customFields.section.toLowerCase()+'"');
  }
  else {
	 console.log('>>>> typeof media_customFields.section is not a string');
  }
 
  if(typeof media_customFields.subsection == 'string') { 
     media_franchise   = media_customFields.subsection.toLowerCase();
	 console.log('>>>> subsection "'+media_customFields.subsection.toLowerCase()+'"');	 	
  }       
  else {
	 console.log('>>>> typeof media_customFields.subsection is not a string');
  }  

/* New Metrics Area------------------------------*/    
  var media_published_date=''; 
  console.log('> getting published date now');
  try {  
    get_published_date=evt.media.publishedDate;
  }
  catch (err) {
	  console.log('>> error * * * trying to get published date = "'+err+'"'); 	  
  }
      console.log('> published date = "'+get_published_date+'"');
  
  if (get_published_date!='published date unavailable') {
	  console.log('> pub date is available!');
	  get_published_date=get_published_date.toString();	  
	  var pdYYYY = get_published_date.split(' ')[3]; // year	  	  	  
	  var pdDD   = get_published_date.split(' ')[2]; // dd
	  var pdMM   = get_published_date.split(' ')[1]; // mm	  
	  var mmArray = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];	  
	  var mmLookup = jQuery.inArray(pdMM, mmArray)*1+1;
	  if (mmLookup.toString().length==1) {pdMM='0'+mmLookup.toString();}	  
	  if (pdDD.length==1) {pdDD='0'+pdDD;}
	  var pdYYYY_MM_DD=pdYYYY+'-'+pdMM+'-'+pdDD;	   
	  media_published_date=pdYYYY_MM_DD;
	  console.log('> media_published_date ="'+media_published_date+'"');  
  }  
...
// Update franchise code for chann=
// media_customFields = evt.media.customFields;
...
/*-----------------------------------------------*/
  ...
  //media_customFields=evt.media.customFields;
  //if(typeof media_customFields.section=='string') { media_franchise=media_customFields.section.toLowerCase();}  
  
  s_time.eVar52 = vidPlayer[currentPlayer.id].publishedDate;   // video publish date 
...
s_time.eVar52 = vidPlayer[playerMilestone.id].publishedDate; // video publish date 	
