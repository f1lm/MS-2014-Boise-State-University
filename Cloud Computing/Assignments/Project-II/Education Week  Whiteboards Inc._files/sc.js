...
var s=s_gi(s_account);s.charSet="AUTO";var dots=hostName.split('.');s.cookieDomainPeriods=dots.length-1;s.currencyCode="USD";s.trackDownloadLinks=true;s.trackExternalLinks=true;s.trackInlineStats=true;
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";s.linkInternalFilters="javascript:,edweek.org,teachermagazine.org,topschooljobs.org,www.teachersourcebook.org,digitaldirections.org";
s.linkLeaveQueryString=false;s.linkTrackVars="eVar41,prop41,prop15,eVar15,events";s.linkTrackEvents="event20";s.pageName=""; s.pageURL = location.toString();

s.usePlugins=true
function s_doPlugins(s) {
	var nl=(!s._preview)?location:new LOCATION(s.getQueryParam('source'));
	var baseURL=nl.hostname.toLowerCase()+nl.pathname.toLowerCase();var path=nl.pathname.toLowerCase();var params=nl.search.toLowerCase();
	if (!s.pageName){
	s.pageName=path.substr(1);s.pageName=s.pageName.replace(/^\//, "").replace(/\/\//g,"/").replace(/index.html/,"").replace(/%20/g," ");
	if(s.pageName.indexOf(s.channel + '/') == 0){if(s.pageName.length == s.channel.length + 1){s.pageName = s.channel + "/home";}}else{s.pageName = s.channel + '/' + s.pageName;}
	if(s.pageName.indexOf('/articles/') != -1 || s.pageName.indexOf('ewblog/') != -1 || s.pageName.indexOf('tmblog/') != -1){t = document.title.split(":");t=t[t.length - 1];n=t.indexOf('#');if(n>-1)t=t.substr(0,n);if(s._preview)t='PREVIEW|'+t;s.pageName += '|' + t;}
	}
	...
	if (!s.prop31) {
		for (index=0; index < metatags.length; index++) {
			...
			if (name && name.indexOf("ID") == 0){
				...
				is_blog = true;
				s.prop31 = ((content.substring(0, 2) != "B:") ? "B:" + content : content);
				break;
			}
			if (name && name.indexOf("uuid") == 0){
				var content = metatags[index].getAttribute("content");
				is_article = true;
				s.prop31 = ((content.substring(0, 2) != "A:") ? "A:" + content : content);
				break;
			}
		}
	}
	...
	if (s.prop31) {
		for (index=0; index < metatags.length; index++) {
			var name = metatags[index].getAttribute("name");
			if (name && name.indexOf("Pub_first") == 0){
				var content = metatags[index].getAttribute("content");
				s.prop35 = content;
				break;
			}
	...
	for (index=0; index < metatags.length; index++) {
		var name = metatags[index].getAttribute("name"); var content = metatags[index].getAttribute("content"); 
		if (name && name.indexOf("Story_type") == 0){
			if (content.indexOf("Story") >= 0) {
				is_article = true;
			} 
		} 
		if (name && name.indexOf("ID") == 0){ 
	...
	if (is_article || is_blog) {
		if (s._preview) { 
			s.eVar13 = null; s.eVar20 = "+1"; 
		} else { 
			s.eVar13="+1"; s.eVar20 = null; 
		} 
	} else {
		s.prop31 = null;
	}			
	...
	if (month < 10){month = "0" + month;}var day = currentTime.getDate();if (day < 10){day = "0" + day;}var year = currentTime.getFullYear();
	s.prop7=year + "-" + month + "-" + day; //Note that this sets the time based on the users system clock date
	s.eVar7='D=c7';

	if(s.pageType && s.pageType=="errorPage"){
		s.pageName="error|"+baseURL;
	}else if (s.pageType && s.pageType=="searchPage"){
		s.pageName="ew/search|page=" + s.pageNum;
		if (s.pageNum < 2)s.eVar14 = "+1";
		s.hier1=s.prop1=s.prop2=s.prop3=s.channel=s.eVar1=s.eVar2=s.eVar3="ew";
		s.pageType = ""; // reset pageType.
	}else if (s.pageName){
		page_split = s.pageName.split("/");
		s.channel=s.prop1=s.prop2=s.prop3=page_split[0];
		s.eVar1=s.eVar2=s.eVar3='D=c1';
		if(page_split.length>2) {
			s.prop1=s.prop2=s.prop3=page_split[0]+"/"+page_split[1];
			s.eVar1='D=c1';
		}
		if(page_split.length>3)s.prop2=s.prop3=s.eVar2=s.eVar3='D=c1+"/' + page_split[2] + '"';
		if(page_split.length>4)s.prop3=s.eVar3='D=c1+"/' + page_split[2] + '/' + page_split[3] + '"';
		s.hier1 = page_split[0];
		for (i=1; i< page_split.length - 1; i++)
		{	
			s.hier1 += "/" + page_split[i];
		}
	}

	s.prop11=s.prop12="Anonymous";
	
	if(s._prof)s.prop12=s._prof;
	if(s._slu)s.prop11="5000";
	else if(s._level)s.prop11=s._level;	
	s.events=s.apl(s.events,'event1',',',1);
	if (s.getQueryParam('r') && s._login)
	{
		var login_key = s.getQueryParam('r');
		login_key = s.getValOnce(login_key,'login_key',0);
		if (login_key) 
		{ 
			s.events=s.apl(s.events, 'event9', ',', 1); 
		}
	}
	if (s.getQueryParam('print') == 1)
		s.events=s.apl(s.events, 'event12', ',', 1);

	s.prop4=s.getTimeParting('h','-5',year);
	s.eVar4='D=c4';
	s.prop5=s.getTimeParting('d','-5',year);
	s.eVar5='D=c5';
    
    s.prop6=s.getDaysSinceLastVisit();
    s.prop6=s.getAndPersistValue(s.prop6,'s_dslv',0);
    s.eVar6 = 'D=c6';
    
    
	if(s.channel) s.prop8=s.eVar8='D=ch';

	s.prop9='D=g';
	s.eVar9='D=g';
    s.prop10 = 'D="' + s.getNewRepeat('','s_gnr')+':"'+'+pageName';

	if(s.prop11){
		s.eVar11='D=c11';
		if(s.pageName)s.prop13='D=c11+":"+pageName';
		
		if(s.prop12 ){
		s.eVar12='D=c12';
		if(s.pageName)s.prop14='D=c12+":"+pageName';
		}
	}
	
	if(s.prop15)s.eVar15='D=c15';
	if(!s.prop16)s.prop16=s.getQueryParam('intc').toUpperCase();
	...
	if(s.prop22)s.eVar22=s.prop22=s.prop22.toLowerCase();
	...
	{
		var name = metatags[index].getAttribute("name");
		if (name && name.indexOf("Category_secondary") == 0)
		{
			var content = metatags[index].getAttribute("content");
			s.prop28 = content.replace(/\;/g, ",");
			s.eVar10 = 'D=c28';
			break;
		}
	}
	...
	if(s.prop35){
		var date_array=s.prop35.split("-");
		if(date_array.length==3){
			var pub_date=new Date(date_array[0], date_array[1]-1, date_array[2]);
			var date_array2=s.prop7.split("-");
			if(date_array.length==3){
				var current_date=new Date(date_array2[0], date_array2[1]-1, date_array2[2]);
		
				var one_day=1000*60*60*24;

				s.prop37=""+Math.ceil((current_date.getTime()-pub_date.getTime())/(one_day)); 
			}
		}
	}

	//if(s.prop36)s.eVar36='D=c36';	 Removed for Adwords
	if(s.prop37 && s.prop37<0)s.prop37="";
	...
	var urchinArray = new Array("utm_source", "utm_medium", "utm_campaign", "utm_content");
	var uLen = urchinArray.length;
	
	if(!s.campaign) {
		s.campaign=s.getQueryParam('cmp').toLowerCase();
		if (s.campaign == '') {
			for (x=0; x < uLen; x++) {
				s.campaign += ((s.campaign != "") ? "|" + s.getQueryParam(urchinArray[x]).toLowerCase() : s.getQueryParam(urchinArray[x]).toLowerCase());
			}
		}

	}
	s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
	if (s.campaign == '') if (s.cmp) s.campaign = s.cmp;	
    if(s.getAndPersistValue(s.campaign,'s_cmp',0)!='') s.prop20 = 'D="'+s.getAndPersistValue(s.campaign,'s_cmp',0) + ':"+pageName'; else s.prop20 = 'D="NONE:"+pageName';
    s.url = s.downloadLinkHandler();
    if (s.url) {
        s.eVar41=s.prop41 = s.url.substring(s.url.lastIndexOf("/") + 1, s.url.length);
        s.prop15=s.eVar15 = s.prop41+":"+s.pageName;
	...
	//WOSC-85
	if (!s.eVar35)s.eVar35=s.getQueryParam('ccid');
	if (!s.eVar36)s.eVar36=s.getQueryParam('ccag');
	if (!s.eVar37)s.eVar37=s.getQueryParam('cckw');
	if (!s.eVar38)s.eVar38=s.getQueryParam('cccv');
...
function LOCATION(locObj){
	...
	this.setFragments = function(frags, commitIt) {
		...
		if(frags) {
			...
			this.host = [this.hostname, this.port].join(':');
		}
	...
	this.decompose = function() {
		this.protocol = locObj.protocol.replace(/:$/, '');
		this.hostname = locObj.hostname;
		this.port     = locObj.port.replace(/^:/, '');
		this.host     = [this.hostname, this.port].join(':');
		this.pathname = '/'+locObj.pathname.replace(/^[/]/, '').replace(/\\/g, '/');
		this.search   = locObj.search;
		this.hash     = locObj.hash;
		if(this.compose() === '') { this.pathname = locObj.href; }//Special case for relative urls
	};
	this.getLink = function() {
		if(!locObj.tagName) {
			var L = new LOCATION(this.compose());
			return L.getLink();
		}
		return locObj;
	};
	this.inspect = function(wrapperID) {
		strArray = [];
...
s.setUser=new Function("u","l","p",""
+"var s=this;if(u&u>0){s._login=true;if(l)s._level=l;if(p)s._prof=p;}return;");
...
s.visitorNamespace="educationweekonline"
s.trackingServer="m.edweek.org"
s.trackingServerSecure="sm.edweek.org"
s.dc="122"
