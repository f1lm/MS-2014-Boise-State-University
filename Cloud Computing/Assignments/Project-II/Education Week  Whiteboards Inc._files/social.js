	...
	function ($, jQuery, dmJQuery){
		...
		if (typeof(pluckAppProxy.pluck_social_facebook) === 'undefined') {
				...
				if(pluckAppProxy.pluck_social_facebook_sessionCache){
					...
					FB.getLoginStatus(function(response){
						...
						if(session){
							...
							var permsString = pluckAppProxy.pluck_social_facebook_requiredPerms.join(',');
							var query = FB.Data.query('select ' + pluckAppProxy.pluck_social_facebook_requiredPerms + ' from permissions where uid={0}', session["uid"]);
							query.wait(function(rows) {
								var allPerms = true;

								for(var i in pluckAppProxy.pluck_social_facebook_requiredPerms){
									var perm = rows[0][pluckAppProxy.pluck_social_facebook_requiredPerms[i]];
									if(!perm || perm != 1){
										allPerms = false;
										break;
									}
			...
			pluckAppProxy.pluck_social_facebook_connect = function(target, callback, cancelCallback) {
				pluckAppProxy.pluck_social_facebook_private_requireSession(function(uid, session) {
					if (callback) callback(uid, session);
				}, function() {
		...
		if (typeof(pluckAppProxy.pluck_social_syndication) === 'undefined') {
			pluckAppProxy.pluck_social_syndication = function (htmlId, parentId, params) {
				...
				pluckAppProxy.registerCssCallback(isIE6 ? "pluck/social/social.ie6.css" : "pluck/social/social.css", function() {
					top.show();
				});
			};
		}
