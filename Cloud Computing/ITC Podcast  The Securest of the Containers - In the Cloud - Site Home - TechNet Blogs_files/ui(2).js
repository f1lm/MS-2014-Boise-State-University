(function ($) {
    ...
    var errorHtml = '<div class="message error">{ErrorText}</div>',
            ...
            $.telligent.evolution.get({
                ...
                success: function (response) {
                    if (response) {
                        ...
                        setTimeout(function () { fixPager() }, 1000);
                    }
                },
        ...
        fixPager = function () {
            var pager = $('.pager.ui-page:first');
            if (pager.length == 1) {
                var pagerLinks = $(pager).children('a');
                $.each(pagerLinks, function (index, value) {
                    var page = $(value).data('page');
                    $(value).attr('onclick', "$(document).trigger('msdntechnet_blogs_commentpagerclicked', '" + $(this).attr('href') + "'); return false;");
                });
            }
        },
    ...
    $.telligent.evolution.widgets.blogFeedbackList = {
        register: function (context) {
            ...
            $(document).bind('msdntechnet_blogs_commentpagerclicked', function (e, message) {
