...
// Updated: 12/29/2011 - "downloadtypes" file extension list update - Line 10
// Updated: 1/25/2012 - added support for FPC 

function WebTrends(){
	var that=this;
	// begin: user modifiable, 
	//set it for production server. the proper dcsid will be assiged when it is built.
	this.dcsid="dcsngbilzcxafpc7vw2qgbbij_3j2v";
	this.domain="wt.research.gov";
	this.timezone=-5;
	this.fpcdom=".nsf.gov";
	this.onsitedoms="www.nsf.gov,nsf.gov,www.dev.nsf.gov,www.acpt.nsf.gov, www.test.nsf.gov";
	this.downloadtypes="xls,doc,pdf,txt,csv,zip,xlsx,docx,ppt,pptx,jpg,png,gif,mp3,swf,flv,mov,wmv,sas7bdat,sav";
...
WebTrends.prototype.dcsCustom=function(){
	//Utilize WebTrends dcsQP function to automatically grab the following NSF query parameters
	//when available.  The dcsQP function will handle errors if they are not present.
	//Brian Gallagher 4/14/2009
	this.WT.z_med_id=this.dcsQP("med_id");
	this.WT.z_from=this.dcsQP("from");
	this.WT.z_org=this.dcsQP("org");
	this.WT.z_cntn_id=this.dcsQP("cntn_id");
	this.WT.z_pims_id=this.dcsQP("pims_id");
	this.WT.z_ods_key=this.dcsQP("ods_key");
	this.WT.z_med_id=this.dcsQP("med_id");
}
WebTrends.prototype.dcsCollect=function(){
    if (this.enabled){
        ...
        this.dcsMeta();
        this.dcsAdv();
		this.dcsCustom(); //Call Custom data capture function, B.G.
        this.dcsTag();
    }
}
