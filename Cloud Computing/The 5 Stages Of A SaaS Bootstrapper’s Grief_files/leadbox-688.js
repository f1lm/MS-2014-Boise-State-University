...
;(function() {
    ...
    if (window.LEADBOX) return; else window.LEADBOX = {};
    ...
    function addInterval(block, delay) {
        var interval = { id:null, loop:null, delay:delay, block:block };
    ...
    var MouseTracker = (function(debugging) {
        ...
        var size       = this.size       = { x:null, y:null };
        var history    = this.history    = { x:null, y:null };
        var position   = this.position   = { x:null, y:null };
        var prediction = this.prediction = { x:null, y:null };
        var velocity   = this.velocity   = { x:0, y:0 };
        ...
        function updateVector() {
            ...
            var projection   = { x:position.x, y:position.y };
            ...
            past = [{ x:velocity.x, y:velocity.y }].concat( past.slice(0, back) );
            ...
            var next = { x:position.x + average.x, y:position.y + average.y };
        ...
        addEvent(canvas, 'mousemove', updatePosition); updatePosition();
        addEvent(window, 'resize', updateSize); updateSize();
    ...
    var LeadBoxes = function() {
            ...
            if (typeof window.postMessage == "undefined" ||
                ...
                leadboxes[id][0].onload = function() {
                    if (window.LEADBOX.debugging) {
                        ...
                        leadboxes[id][0].contentWindow.postMessage({ handler:'reset', data:{id:id}}, '*');
                    }
                    var msg = JSON.stringify({ handler:'visit', visit:{ id:id } })
        ...
        function show(id, cb) {
            ...
            if (typeof leadboxes[id][0] == "string")
                // open leadbox in new tab
                window.open(leadboxes[id][0], "_blank");
            else {
        ...
        setTimeout(function(){ new MouseTracker() });
