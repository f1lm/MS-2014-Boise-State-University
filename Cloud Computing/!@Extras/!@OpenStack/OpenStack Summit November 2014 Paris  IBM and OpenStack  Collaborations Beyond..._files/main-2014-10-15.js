...
if ($('#js-settings-form').length) {
  ...
  $('#js-settings-form').submit(function() {
    if (!cv && oun != $('#sched-username').attr("value")) {
      cv = true;
    ...
    if (cv) {
      ...
      } else if ($.trim($('#pass-confirm').val()) && oun != $('#sched-username').attr("value")) return true;
      else if (oun == $('#sched-username').attr("value")) return true;
      else {
      
        // does the user need to set up a new password?
        if ($('#pwone').length) {
          if (  
              $.trim($('#pwone').val()) == '' || $.trim($('#pwtwo').val()) == ''
              || $.trim($('#pwone').val()).length < 3               
              ) { alert('Please enter password'); $('#pwone').focus(); return false }
          if ($('#pwone').val() != $('#pwtwo').val()) {
            alert("The passwords don't match"); return false;
          }
        } else {
...
function saveEvent(lnk, removecheck) {
  ...
  if(username != "")
  {
    ...
    if ($('#page-event').length) {
      if ($('.sub').length) {
        $('#sched-page-event-attendees').prepend('<p class="sched-page-event-attendees-note">Attendance numbers do not account for private attendees. Get there early!</p>');      
      } else {
    ...
    else
    {
      ...
      if($("#" + id + " .sched-event-seats").length != 0)
        ...
        var seats = $("#" + id).next().children(".sched-event-seats");
      var pseats = $("#" + id).parent().parent().find(".sched-event-permalink-seats");
      ...
      if(prli.hasClass("sub"))
      {
        ...
        if((seats.hasClass("sched-event-seats-full") || seats.hasClass("sched-event-seats-filling")) && removecheck) {
          $("#sched-overlay-generic").show();
          $("#sched-overlay-removeconfirm").show();
          $("#sched-overlay-generic-inner").children("#sched-overlay-button-red").off('click').click(function() {
            saveEvent(id, false);
            $(this).off('click');
        ...
        $.get("/event-set?del=" + id, function(data) {
          ...
          if(data == "noremove") {
            ...
            if(isEmbed)
              $("#sched-overlay-inner").css({top: (prli.offset().top+($("#sched-overlay-inner").height()/2)) + 'px'});
            $("#sched-overlay").fadeIn(200);
          }
          if(data == "free" || data == "filling" || data == "full") {
            $("#sched-page-event-addbutton a").html("&#9733; Add To My Sched");
            ...
            if(data == "full") psnote.html("Adding this to your schedule will put you on the wait list.");
            if(data != "full" && data != "waitlisted") psnote.hide();
            if(tips[id] != undefined) delete tips[id];
          }
          $.each(slot, function(i, v) {
            $("#"+v).parent().fadeTo("slow", 1.0);
          });
        });
      }
      else
      {
        ...
        $.get("/event-set?add=" + id, function(data) {
          ...
          if(data == "countexceeded" || data == "expired" || data == "noticket" || data == "waitlisted" || data == "waitlisted-no" || data == "no-slot") {
            if(data != "waitlisted") {
              prli.removeClass("sub");
              hasSchedule--;
              if(hasSchedule < 1) {
            ...
            if (message) {
              $("#sched-overlay-message").html(message+'<br />').show();
            } else {
              $("#sched-overlay-"+data).show();
            }
          ...
          if(data == "free" || data == "filling" || data == "full" || data == "waitlisted") {
            $("#sched-page-event-addbutton a").html("&#9734; Remove From My Sched");
            if(data == "free") data = "limited";
            seats.removeClass();
            seats.addClass("sched-event-seats");
            seats.addClass("sched-event-seats-"+data);
            ...
            pseats.html(seatsstring);
            if(data == "waitlisted") psnote.html("You're on the wait list.");
            if(data == "full") psnote.show();
          }
          $.each(slot, function(i, v) {
            $("#"+v).parent().fadeTo("slow", 0.4);
          });
        });
      }
    }
...
function showToolTip(lnk)
{
  ...
  if(tips[id] == undefined)
  {
    ...
    tooltipwait = setTimeout(function() {
      $("#tip").html("<div class='popover'><div class='popover-content'><div class='arrow'><span></span></div><div class='popover-body'><div class='popover-body-inner'><div class='sched-meta-loading'><img id='tip-loading' src='\/\/cdn.schd.ws\/common\/loading.gif' \/></div></div></div></div></div>").show().css({left: tx + "px", top: ty + "px"});
...
var tb_pathToImage = "//cdn.schd.ws/common/loading.gif";
...
function toggleExpired(caller) {
  $('.sched-container-hidden').slideToggle(
    function(){
      if($('.sched-container-hidden').is(":visible")) caller.innerHTML = "↑ Hide Expired ↑";
      else caller.innerHTML = "↓ Show Expired ↓";
    }
  );
}
...
function changeTipGrav() {
  ...
  if(tgnew != false) {
    tg = tgnew;
    $('a.sched-title-tooltip').qtip({style:{classes:'ui-tooltip-bootstrap'},position:{my:tg[0],at:tg[1],target:'a[title]'}});
  }
}

$(function(){

    $("#schedorg-header-profile li").hover(function(){
