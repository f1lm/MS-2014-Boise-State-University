...
$(function(){
	...
	$('.select_link, .top-nav a.top-nav-select-site').click(function() {
	...
	$('.multistory-list-button a, a.multistory-collapse-button').click(function() {
		$('.multistory-list-button').toggle();
		$('.multistory-list-content').toggle();
		return false;
	});

	$('.topic-intro-link').click(function() {
		$('.topic-intro-more').toggle();
		if ($('.topic-intro-more').is(':visible')) {
			$(this).html('Less...');
		} else {
			$(this).html('More...');
		}
	...
	$('.popular_stories_tabs li a').click(function() {

		$('.popular_stories_tabs li a').removeClass('active');
		$('#popular_most_read').hide();
	...
	/* Check for height of main column on story page and display proper set of share buttons */
	if ($('.maincolumn').length && $('.share_buttons_large').length && $('.share_buttons_small').length) {
		if ($('.maincolumn').height() < 789) {
			$('.share_buttons_large').hide();
			$('.share_buttons_small').show();
		}
	}

	$('.search-form-header .text').focus(function() {
		if ($(this).val() == 'SEARCH') {
	...
	$('#fierce-library-search-form #edit-search').click(function() {
		if ($(this).val() == 'Search The Library') {
			$(this).val('');
		}
	});


	$('#sidebar .text, .holder .text, .content-subscribe .text').focus(function() {
		if ($(this).val() == 'EMAIL ADDRESS') {
			$(this).css({color: '#000000', backgroundColor: '#ffffff'});
			$(this).val('');
		}
	});
	$('#sidebar .text, .holder .text, .content-subscribe .text').blur(function() {
		var trim = $(this).val().replace(/[\s]/g, '');
		if(trim === '') {
	...
	$('#sidebar .btn-submit').click(function() {
		var val = $('#sidebar .text').val();
		if(val.search(/[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/gi) === -1) {
			$('#sidebar .text').css('border', '#ff6600 1px solid');
			$('#sidebar .text').focus();
			return false;
		}
		return true;
	});
	$('.holder .btn-submit').click(function() {
		var val = $('.holder .text').val();
		if(val.search(/[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/gi) === -1) {
			$('.holder .text').css('border', '#ff6600 1px solid');
			$('.holder .text').focus();
			return false;
		}
		return true;
	...
	$('.content-subscribe .btn-submit').click(function() {
		var val = $('.content-subscribe .text').val();
		if(val.search(/[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/gi) === -1) {
			$('.content-subscribe .text').css('border', '#ff6600 1px solid');
			$('.content-subscribe .text').focus();
			return false;
		}
		return true;
	});

	//scroll down to site list
	$('#top_select .more, #mid_select .more').click(function(event) {
        event.preventDefault();
		$('html, body').animate({
			scrollTop: $("#more-fierce-network-sites").offset().top
		}, 'slow');
		return false;
    });

    if(mql) {
    	$('.sharecolumn').appendTo('.maincolumn');
    }
