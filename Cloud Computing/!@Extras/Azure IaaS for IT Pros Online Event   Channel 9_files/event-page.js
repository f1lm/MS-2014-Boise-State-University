$.lald('.clearAll', 'click', function () {
	$('.sidebar .search input:checkbox').prop('checked', false);
	$('.sidebar input:text').val('');
	return false;
});

$.lald('.submitFilter', 'click', function () {
	$('.sidebar form').submit();
	return false;
});
...
$.lald(".c9wtMCID", "click", function () {
	var wtMCID = getParamByName("WT.mc_id");
	if (wtMCID) {
		dcsMultiTrack("WT.mc_id", wtMCID);
	}
	return true;
});
