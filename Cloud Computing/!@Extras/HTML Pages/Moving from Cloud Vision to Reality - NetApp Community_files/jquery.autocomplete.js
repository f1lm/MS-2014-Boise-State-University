...
$.Autocompleter = function(input, options) {
	...
	function receiveData(q, data) {
		...
		$input.trigger("autocompleteReceiveData", data);
	};
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	function fillList() {
		...
		for (var i=0; i < max; i++) {
			...
			if ( formatted === false )
				continue;
			var li = $("<li/>").html( options.highlight(formatted, term) ).addClass(i%2 == 0 ? options.evenRowClass : options.oddRowClass).appendTo(list)[0];
