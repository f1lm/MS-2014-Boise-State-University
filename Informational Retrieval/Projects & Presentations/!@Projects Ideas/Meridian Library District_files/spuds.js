...
$Trumba.showWindow=function(url,target,features){var newWindow=window.open(url,target,features);newWindow.focus();return newWindow;}
...
if(resize)
this.resize();},onSpudLoaded:function(){this.container.onSpudLoaded();},_getEmbedArguments:function(){var qs1=new $Trumba.Net.QueryString(location.search);var qs=qs1.getAt("trumbaEmbed");if(qs==null||qs.length==1){try{if(window.top.location.search!=location.search){qs1=new $Trumba.Net.QueryString(window.top.location.search);qs=qs1.getAt("trumbaEmbed");if(qs==null||qs.length==1)
return null;}
else
return null;}catch(e){return null;}}
if(qs[1]=='view=event'){var qsid=qs1.getAt("eventid");if(qsid!=null&&qs.length>1)
qs[1]+='&eventid='+qsid[1];}
...
$Trumba.baseUri+'images/spinner.gif" width="16" height="16">&nbsp;&nbsp;Loading&nbsp;.&nbsp;.&nbsp;.</div>';this.container.setHTML(html,true);this.container.resize();},_getBusyImageId:function(){return this.id+".busy";},_showBusyImage:function(pos){var hideBusyImage=this.getProperty("hideBusyImage")||false;if(!hideBusyImage){var imgLoading=$Trumba.DOM.createElement("img",[["id",this._getBusyImageId()]]);imgLoading.style.position="absolute";imgLoading.style.left=pos.left+"px";imgLoading.style.top=pos.top+"px";imgLoading.style.width="16px";imgLoading.style.height="16px";imgLoading.style.zIndex=100000;imgLoading.src=$Trumba.busyImageUri;document.body.appendChild(imgLoading);$Trumba.logger.info(this._getBusyImageId()+", parent:"+imgLoading.parentNode.tagName,null,"$Trumba.Spuds.SimpleSpud","_showBusyImage");}},_hideBusyImage:function(){var imgLoading=$Trumba.$(this._getBusyImageId());if(imgLoading){$Trumba.logger.info(this._getBusyImageId()+", parent:"+imgLoading.parentNode.tagName,null,"$Trumba.Spuds.SimpleSpud","_hideBusyImage");imgLoading.parentNode.removeChild(imgLoading);}}}
