...
__sco.config = {
    ...
    'guid': '19c3b3ad-7b43-4b08-9c6a-fe1f7964d352', // V2 guid
    'v1guid': '19c3b3ad-7b43-4b08-9c6a-fe1f7964d352', // V1 guid, the api key for the client

    'triggers': ['exit', 'timeout'], // Set the triggers to use, use either load or exit and timeout together

    'status1': ["/s.nl/sc.3", "sc=4", "/s.nl?sc=3", "s.nl?sc=4", "s.nl/sc.4/.f"], // Strings to be set in lower case, will be compared against the URL of the current page. Functions will be executed and must return either true, false or a number to use as the status
    'status2': ["STATUSTWO"],
    'status3': ["category=thanks&whence"],
    'status4': ["STATUSFOUR"],
    'exclude': ["RUN ON EVERYTHING"],

    'onchange': {
        'email': ["#email", "#retemail"],
        'first': ["#name"]
    },
    'block': {},
...
__sco.scraper.statusone = function () {
    ...
    //DYNAMIC ROUTE
    if (__sco.contains(__sco.loc, "/s.nl/sc.3") || __sco.contains(__sco.loc, "/s.nl?sc=3") || __sco.contains(__sco.loc, "s.nl?sc=4") || __sco.contains(__sco.loc, "s.nl/sc.4/.f")) {
        try {
            if (__sco.contains(__sco.loc, "s.nl?sc=4")) {
                __scd.b.v = _scs("#header-cart-total", "Total Price - Checkout", ["text", "pricecurr"]);
            }
            else if (__sco.contains(__sco.loc, "/s.nl/sc.3") || __sco.contains(__sco.loc, "/s.nl?sc=3")) {
                __scd.b.v = _scs("#header-cart-total", "Total Price - Checkout", ["text", "pricecurr"]);
            }
        }
        catch (be2) {
        ...
        try {
            __sco.each(_scs("#carttable tr"), function (ix, val) {
                var src = "(http://|https://)", href = "(http://|https://|store.mentalfloss.com)";

                // Set basket values here
                if (_scs([val, "img:first"]) && _scs([val, ".dottedlink:first"])) {

                    if (_scs([val, "img:first"])) {
                        if (_scs([val, 'input.input:first'])) {
                            var quantity = __sco.getvt(_scs([val, "input.input:first"], "Item Qty"));
                        }
                        else {
                            var quantity = _scs([val, ".listtexthl:first"], "Item Qty", ["text"]);
                        }
                    }

                    __scd.b.i.push({
                        'n': _scs([val, ".dottedlink:first"], "Item Name", ["text"]),
                        'i': __sco.clear(_scs([val, ".dottedlink:first"], "Item Name", ["text"]), " ", "g", "-"),
                        'q': quantity,
                        'v': _scs([val, ".texttablert:first"], "Item Price", ["text", "pricecurr"]),
                        'f1': __sco.clear(__sco.attr(_scs([val, ".dottedlink:first"]), "href", href, "g")),
                        'f2': '-_-DYNAMIC-_-',
                        'u': __sco.attr(_scs([val, 'img:first']), "src")
                    });
                }
            });
        }
    ...
    //STATIC ROUTE
    if (__sco.contains(__sco.loc, "?sc=4") && (_scs(".greytitle:first", "Title Check", ["text"]) != "Review and Submit Your Order")) {
        if (__sco.contains(document.referrer, "/s.nl/sc.3") || __sco.contains(document.referrer, "/s.nl?sc=3")) {
            //Do nothing if you have come from the shopping cart page
            var Nothing = "True";
        }
        else {
            try {
            ...
            try {
                __sco.each(_scs("body"), function (ix, val) {
                    var src = "(http://|https://)", href = "(http://|https://)";
                    // Set basket values here
                    __scd.b.i.push({
                        'n': 'Basketless Session',
                        'i': 'Basketless-Session',
                        'q': 1,
                        'v': 40.00,
                        'f1': '',
                        'f2': '-_-STATIC-_-',
...
try {
    ...
    __sco.osr.clientConfigModule = function () {
        meld.around(__sco.osr, "preShowChecks", function (methodCall) {
            ...
            if (document.location.href.indexOf('category=thanks') != -1) {
                return false;
            }
            return true;
        });
