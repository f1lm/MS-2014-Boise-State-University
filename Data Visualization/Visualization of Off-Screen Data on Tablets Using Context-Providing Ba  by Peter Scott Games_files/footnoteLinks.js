...
YUI().use('node', 'cookie', 'event-base', function (Y) {
	...
	function initialLoad() {
		...
		if (
			...
			&& ( (deviceWidth < 801) && (deviceWidth > 699) ) // If the device is tablet-sized
		) {
			addSwitcher();
		};
	...
	function headShrinker() {
		if ( Y.UA.ie == 0 || Y.UA.ie >= 9  ) {
			...
			// Position the navigation, if it's below the header
			if (Y.one('#navigation')) {
				if (defaultStyle == "mbl") {
					Y.one('.mbl #navigation').setStyle('top', myNewHeaderMeasurements['header'].height);
				}
			}

			// Add room for the fixed header
			var headerHeight = myNewHeaderMeasurements['header'].height;
			if (Y.one('#navigation')) { headerHeight += myNewHeaderMeasurements['navigation'].height };
			if (defaultStyle == "mbl") {
				Y.one('body.mbl').setStyle('paddingTop', headerHeight);
			} else if (defaultStyle == "desktop") {
				Y.one('body.desktop').setStyle('paddingTop', bodyPaddingTop);
			}
