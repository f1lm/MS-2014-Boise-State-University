 ...
 * @param {Boolean} [forceSecureAssets=false]
 */
window.GrvWidgetVars = function(siteGuid, placementId, userGuid, truncateArticleTitles, truncateArticleContent, isSidebar,
  ...
  if(this.dataLayerEnvironmentIsolated == null)
    this.dataLayerEnvironmentIsolated = false;
  if(this.omnitureEnvironmentIsolated == null)
    this.omnitureEnvironmentIsolated = false;

  this.useHttps = function() {
    return location.protocol == 'https:' || forceSecureAssets;
  };
};
...
GrvWidgetVars.prototype.stylesLoadedPromise = function($) {
  if(!this._stylesLoadedDeferred) {
    ...
    var $stylesheets = $('link.grvWidgetStylesheet.sg' + this.siteGuid + '-pl' + this.placementId),
        $stylesheet = $stylesheets && $stylesheets.filter(isSitePlacementSpecificCssOverride);
 ...
 */
window.grvInitWidget = function($, widgetVars, forceReinit) {
  // De-dupe execution of this just in case...
  this.inittedWidgets = this.inittedWidgets || {};
  forceReinit = forceReinit === true;
  if(!forceReinit && this.inittedWidgets[widgetVars.siteGuid] && this.inittedWidgets[widgetVars.siteGuid][widgetVars.placementId]) {
    console.warn('grvInitWidget called for already initted widget ' + widgetVars.siteGuid + ' ' + widgetVars.placementId);
    return;
  }
  ...
  var forwardHrefDataAttrName = widgetVars.skipRedirectServlet && !window.grvJsonpWidgetLoaderHealthy ? 'data-non-forward-href' : 'data-forward-href';
  ...
  (function() {
    ...
    function thumbyUrl(thumbyDim, thumbyGravity, imageUrl) {
      ...
      return (widgetVars.useHttps() ? 'https://grvaol-a.akamaihd.net' : 'http://a.rtb.grvcdn.com') + '/t/' + widthHeightStr + '/' + thumbyGravity + '/?url=' + encodeURIComponent(imageUrl);
    }
...
//        return;
      ...
      if((widgetVars.useThumby || $link.data('campaignUsesThumby')) && !isCustomGravityCroppedImage) {
        ...
        if(newImgRequired && width && height && imageUrl) {
          ...
          $img = $('<img class="grv_article_img grv_positionable" />')
              ...
              .bind('rescaleImage.grv', $.proxy(scaleThisImgLink, this))
    ...
    var onImageError = function(width, height) {
      ...
      if(width && height) {
        var $img = $(this),
            brokenImgThumb = (widgetVars.useHttps() ? 'https://grvaol-a.akamaihd.net' : 'http://a.rtb.grvcdn.com') + '/t/' + width + 'x' + height + '/Center/?url=' + encodeURIComponent(widgetVars.brokenImgUrl);
  ...
  function bindArticleHandlers($grvparent) {
    ...
    if(/[&?]cc(&|$)/.test(location.search || '') && widgetVars.isIframe && postMessageSupported) {
      ...
      $forwardingLinks.click(function(e) {
        ...
        function passToPartnerClickHandler() {
        ...
        // Trigger a background click beacon only if not skipping redirects
        if(widgetVars.skipRedirectServlet)
          passToPartnerClickHandler();
        else {
          $.ajax(gravRedirectUrl, {
            dataType: 'jsonp',
            jsonp: 'jscb',
            complete: passToPartnerClickHandler
          });
        }
      });
    }
  ...
  function omniTrackLinkClick(e) {
    ...
    var $link = $(this),
        ...
        redirectUrl = widgetVars.skipRedirectServlet ? destUrl : $link.attr(forwardHrefDataAttrName),
  ...
  function gaTrackOrganicClick(e) {
    var $link = $(this),
        ...
        redirectUrl = widgetVars.skipRedirectServlet ? $link.attr('href') : $link.data('forwardHref'),
        newTab = $link.attr('target') == '_blank',
        gaDo = 'send',
        gaType = 'event',
        gaCategory = 'Gravity',
  ...
  function rewriteHref() {
    ...
    if(targetHref && !widgetVars.skipRedirectServlet)
  ...
  function checkRespWidth() {
    if(ensure$widget()) {
      var $subjectEl = widgetVars.isIframe || widgetVars.isSidebar ? $widget : $widget.parent(),
          curWidth = $subjectEl.outerWidth(),
          lastWidth = $widget.data('lastWidthForCheckRespWidth.grv');

      // If couldn't get a width from an el other than widget, then just use widget's width
      if($subjectEl !== $widget && curWidth <= 1)
  ...
  $(function() {
    ...
    $doc.bind('newArticleImgLink.grv', function(e, $elem) { $grvImgLinks = $grvImgLinks.add($elem); });
    ...
    $doc.bind('newArticleTitle.grv', function(e, $elem) { $articleTitles = $articleTitles.add($elem); });

    $articleContents = scoped$('.grv_article_content');
    $doc.bind('newArticleContent.grv', function(e, $elem) { $articleContents = $articleContents.add($elem); });
    ...
    if (widgetVars.isIframe) {
      ...
      widgetVars.doAolOmniture && aolOmnitureAndDataLayerPingIsolatedEnvironment();
    }
    // Non-iframe widget
    else {
      grvWidgetDimUpdated();
      ...
      if(widgetVars.doAolOmniture) {
        if(widgetVars.dataLayerEnvironmentIsolated && widgetVars.omnitureEnvironmentIsolated)
          aolOmnitureAndDataLayerPingIsolatedEnvironment();
      }
    }
  ...
  /**
   * This is only to be used for widgets which don't already have Omniture/DataLayer resources loaded by a third party
   * (usually iframe widgets), ergo "isolated".
   */
  function aolOmnitureAndDataLayerPingIsolatedEnvironment() {
    window.bN_cfg = {
    ...
    (function(){
      ...
      s.src = (widgetVars.useHttps() ? 'https://s' : 'http://o') + '.aolcdn.com/os_merge/?file=/aol/beacon.min.js&file=/aol/omniture.min.js';
