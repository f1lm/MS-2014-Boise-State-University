		...
		notifications: function() {
			...
			var timeFormat = function(time){
				...
				if(seconds < 60*1000) {
					...
					return Math.ceil(seconds/(60*1000))+'m ago';
				}
				else if(seconds < 86400*1000) {
					return Math.ceil(seconds/(3600*1000))+'h ago';
				}
				else {
			...
			return {
				init: function(){
					...
					$('.TNW_global_header').on('click', '.b-TNWgh-toggler.is-notifications.is-unread', function() {
						$.ajax({
							type: "POST",
		...
		topbar: function() {
			...
			return {
				...
				init: function() {
					...
					$('.TNW_global_header').on('click', '.mobile-switcher', 'a', tnw.topbar.onClickToggleSwitcher);
					
					$('.TNW_global_header').on('mouseover', '.b-TNWgh-toggler', tnw.topbar.getAuthPopup);
					$('.TNW_global_header').on('click', '.b-TNWgh-toggler > a', '.b-TNWgh-toggler', tnw.topbar.onClickToggle);
				...
				onClickToggle: function(e) {
					...
					if (!$parent.hasClass('active')) {
						$(".platform").find('.active:not(.search)').removeClass('active');
						$parent.addClass('active');
						e.stopPropagation();
