...
s.linkInternalFilters="javascript:,dc.stevens.edu,//webcampus.stevens.edu,stevens.edu/sit/graduate"
...
/* External Campaign Tracking */
  if(!s.campaign) s.campaign=s.getQueryParam('camp');  // Please change or add parameter to match.
  s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
  
/* This automatically captures the search term into s.prop7 by using the getQueryParam plugin*/ 
    if(!s.prop7) s.prop7=s.getQueryParam('q,terms');   // Please change or add parameter to match.
  
/* This bit of checks to see if s.prop7 has been set, if it has it copies the value into eVar8 and
  coverts everything to lowercase.  Event5 is then automatically incremented after the data has been 
  deduplicated by the getValOnce plugin. */ 
    
  if(s.prop7){
  s.eVar8=s.prop7=s.prop7.toLowerCase();
...
/* Enhanced download tracking */
	...
	if(s.dlurl){
	s.eVar6=s.prop6=s.dlurl.substring(s.dlurl.lastIndexOf("/")+1,s.dlurl.length);
	s.events=s.apl(s.events,"event4",",",2);                
	s.linkTrackVars="eVar6,prop6,events"
	s.linkTrackEvents="event4" 
	}
  
/* Automatically populate s.eVarX with s.propX */
	if(s.prop1&&!s.eVar1) s.eVar1=s.prop1;    // Domains	
  
  
/* New/Repeat Visitor Status */
  s.prop2=s.eVar2=s.getNewRepeat();
  
/* Set Time Parting Variables */
	var currentYear = new Date().getFullYear( );
	  s.prop3=s.eVar3=s.getTimeParting('h','-5',currentYear); // Set hour 
		s.prop4=s.eVar4=s.getTimeParting('d','-5',currentYear); // Set day
		s.prop5=s.eVar5=s.getTimeParting('w','-5',currentYear); // Set Weekend / Weekday 
	
/* Set s.pageName to lower case */
	s.pageName=s.pageName.toLowerCase();
	  
/* Set s.hier1 with s.pageName */
...
s.visitorNamespace="stevens"
s.trackingServer="stevens.122.2o7.net"
