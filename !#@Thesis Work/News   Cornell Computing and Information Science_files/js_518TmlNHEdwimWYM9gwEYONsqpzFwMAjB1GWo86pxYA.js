...
(function($) {
	...
	Drupal.behaviors.awpCISAlert = {
		attach: function(cx, s) {
			var block   = $('#block-views-announcement-block');
			var alerts  = $("[class^='announcement-']", block);
			var cookies = $.cookie('cis-announcements') || {};
			var minutes = 180; // expire after 3 hours.
			...
			alerts.each(function(i, el) {
				var date = new Date;
				var row = $(el).closest("[class^='announcement-']");
				var id = row.attr('class');

				if (cookies[id] && cookies[id].status == 'closed') {
					row.addClass('closed');
				}
				$('.announcement-close').click(function(e) {
					...
					if (!!id) {
						cookies[id] = { status: 'closed' };
						row.addClass('closed');

						$.cookie('cis-announcements', JSON.stringify(cookies), {
							expires: date.setTime(date.getTime() + (minutes * 60 * 1000))
						});
					}
				});
			});
